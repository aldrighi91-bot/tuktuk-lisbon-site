...
function applyPhonePatch(patch, phone, phoneSkipped) {
  if (phone) {
    patch.phone = phone;
  ...
  if (phoneSkipped) {
    patch.phoneSkipped = true;
    patch.phoneConsent = false;
    patch.contactPreference = 'email';
  }
}

function isPhoneSkipped(message = '') {
  const text = normalizeText(message);
  return /\b(skip|email only|no phone|no whatsapp|prefer email|later|not now|none|n\/a)\b/.test(text);
}
...
function mergeLead(existing = {}, message = '', expectedField = '') {
  ...
  const phoneSkipped = isPhoneSkipped(message);
  ...
  if (expectedField && LEAD_EXPECTED_FIELDS.includes(expectedField)) {
    ...
    applyPhonePatch(patch, phone, phoneSkipped);
...
function capacityNote(guests) {
  ...
  if (count <= 6) return 'Your group fits within one tuk tuk capacity, but date and time still need confirmation.';
  if (count <= 12) return 'Each tuk tuk seats up to 6 guests and Tuk Tuk Lisbon operates 2 tuk tuks. Natanael needs to confirm the right setup and availability for your date.';
  return 'For more than 12 guests, Natanael should confirm whether a custom arrangement is possible.';
}
...
function buildTourRecommendation(tour, lead = {}, lang = 'en') {
  ...
  if (lang === 'pt') {
    return [
      ...
      note ? `\n${note}` : '',
      'Posso pedir data, horario, numero de pessoas e pickup para o Natanael verificar disponibilidade.'
    ].filter(Boolean).join('\n');
  }
  ...
  return [
    ...
    note || '',
    'I can collect your date, preferred time, group size and pickup area so Natanael can check availability.'
  ].filter(Boolean).join('\n');
}
...
function needsOptionalPhone(lead = {}) {
  return !lead.phone && lead.phoneSkipped !== true;
}
...
function nextLeadQuestion(lead = {}, lang = 'en') {
  ...
  if (needsOptionalPhone(lead)) {
    return lang === 'pt'
      ? 'Opcional: qual telefone devemos usar para atualizacoes por SMS ou WhatsApp sobre este pedido? Se preferir so email, responda "email only".'
      : 'Optional: what mobile number should Natanael use for SMS or WhatsApp updates about this request? Reply "email only" to skip.';
  }
  return null;
}
...
function nextExpectedLeadField(lead = {}) {
  ...
  if (needsOptionalPhone(lead)) return 'phone';
  return '';
}
...
function quickRepliesForLeadField(field = '') {
  ...
  if (field === 'phone') return ['Email only', 'I will add it later'];
  return [];
}
...
function answerKnownQuestion(message = '', lead = {}, lang = 'en') {
  ...
  if (tour && /\b(price|cost|how much|duration|how long|included|pickup|pick up|hotel|people|capacity)\b/i.test(message)) {
    ...
    parts.push('Availability is checked manually, so I should not promise a date or time without confirmation.');
    return {
      reply: parts.join('\n\n'),
  ...
  if (/\b(pay|payment|deposit|card|cash)\b/i.test(message)) {
    return {
      reply: 'The tour pages mention payment at the end and no deposit, while the Terms page says some bookings may require a deposit to secure confirmation. Natanael should confirm the exact payment details for your booking.',
...
function availabilityReply(lead = {}, lang = 'en') {
  ...
  const intro = lang === 'pt'
    ? 'Posso recolher os detalhes no mesmo padrao da Lisa: tour/zona, data, horario, pessoas, pickup e contato. Nao vou prometer disponibilidade sem confirmacao.'
    : 'Let me check availability for you. I will collect the same booking details Lisa asks for: tour/area, date, time, guests, pickup and contact. I will not promise availability until Natanael confirms it.';
  ...
  return {
    reply: [
      ...
      note ? `\n${note}` : '',
      question ? `\n${question}` : '\nI have the key details. You can send this request to Natanael now.',
    ].filter(Boolean).join('\n'),
...
function continueLeadCapture(lead = {}, lang = 'en', qualification = 'WARM') {
  ...
  return {
    reply: 'I have the key details. You can send this request to Natanael now, and he will personally confirm availability and next steps.',
...
function handleConciergeMessage(input = {}) {
  ...
  if (parseEmail(message) && missingLeadFields(mergedLead).length <= 1) {
    if (needsOptionalPhone(mergedLead)) {
      return continueLeadCapture(mergedLead, lang, intent === 'INFORMATIONAL' ? state.qualification || 'WARM' : intent);
    }
