...
function leadSummaryLines(payload) {
  return [
    ...
    ['Bókun checkout link', payload.bookingCheckoutUrl],
...
function buildOwnerNotification(payload) {
  ...
  const text = [
    ...
    '',
    payload.bookingCheckoutUrl ? `Bókun checkout link: ${payload.bookingCheckoutUrl}` : '',
    payload.bookingCheckoutUrl ? '' : '',
  ...
  const html = [
    ...
    payload.bookingCheckoutUrl
      ? `<p><a href="${escapeHtml(payload.bookingCheckoutUrl)}">Open Bókun checkout for this tour</a></p>`
      : '',
...
function buildCustomerConfirmation(payload, contactEmail = DEFAULT_CONTACT_EMAIL) {
  ...
  const text = [
    ...
    '',
    payload.bookingCheckoutUrl ? `You can also continue with secure online checkout here: ${payload.bookingCheckoutUrl}` : '',
    payload.bookingCheckoutUrl ? '' : '',
  ...
  const html = [
    ...
    payload.bookingCheckoutUrl
      ? `<p>You can also continue with secure online checkout here: <a href="${escapeHtml(payload.bookingCheckoutUrl)}">${escapeHtml(payload.bookingCheckoutUrl)}</a></p>`
      : '',
