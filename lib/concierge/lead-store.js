...
const { sendLeadNotificationEmails } = require('./lead-notifications');

const DEFAULT_SUPABASE_EDGE_LEAD_URL = 'https://fxmxcgqrbwvxnwejasqk.supabase.co/functions/v1/tuktuk-site-lead';
const DEFAULT_SITE_URL = tourData.business?.site || 'https://www.tuktuklisbon.tours';
...
function buildLeadPayload(input = {}) {
  ...
  const bookingCheckoutUrl = buildBookingCheckoutUrl(lead.tourId);
  return {
    ...lead,
    id: crypto.randomUUID(),
    createdAt: new Date().toISOString(),
    source: 'site_concierge',
    tourName: getTourName(lead.tourId),
    bookingCheckoutUrl,
  };
}
...
function buildBookingCheckoutUrl(tourId) {
  const cleanTourId = cleanString(tourId, MAX_FIELD_LENGTHS.tourId);
  if (!cleanTourId) return '';
  return `${DEFAULT_SITE_URL.replace(/\/$/, '')}/booking.html?tour=${encodeURIComponent(cleanTourId)}`;
}
...
async function deliverLead(payload, context = {}) {
  ...
  try {
    const email = await sendLeadNotificationEmails(payload);
    if (email) deliveries.push(email);
  } catch (error) {
    console.error(`concierge lead email notification failed id=${payload.id}`, error.message);
    deliveries.push({ delivery: 'email_failed', error: error.message });
  }
