...
const { getBookingByConfirmationCode, searchBookings } = require('./client');
...
const MAX_SYNC_LOOKBACK_HOURS = 24 * 14;
const DEFAULT_SYNC_PAGE_SIZE = 20;
const MAX_SYNC_PAGE_SIZE = 50;
const DEFAULT_SYNC_MAX_PAGES = 2;
const MAX_SYNC_MAX_PAGES = 5;
...
function buildDateRangeFilter(from, to) {
  return {
    from: formatIsoDateTime(from),
    to: formatIsoDateTime(to),
    includeLower: true,
    includeUpper: true,
  };
}
...
function getBokunLeadId(booking) {
  const stableKey = booking.bookingReference ||
    booking.bookingId ||
    booking.productConfirmationCode ||
    `${booking.tourId}:${booking.desiredDate}:${booking.preferredTime}:${booking.email}`;
  return uuidFromText(`tuktuk-lisbon-bokun:${stableKey}`);
}
...
function extractBookingSearchItems(data) {
  if (Array.isArray(data)) {
    return data.flatMap((item) => extractBookingSearchItems(item));
  }
...
async function getFullBookingForSearchItem(item) {
  const lookupId = bookingLookupId(item);
  if (!lookupId) return { booking: item, lookupId: '', fetched: false };
  ...
  return { booking: item, lookupId, fetched: false, status: response.status };
}

async function processBokunBookingSearchItem(item, context = {}) {
  const detail = await getFullBookingForSearchItem(item);
  return processBokunBookingWebhook({
    event: 'booking.sync',
    booking: detail.booking,
    confirmationCode: detail.lookupId,
  }, {
    ...context,
    headers: {
      ...(context.headers || {}),
      'x-bokun-topic': 'booking.sync',
    },
  });
}

function buildBookingSearchQuery(options = {}, page = 0) {
  const now = options.now instanceof Date ? options.now : new Date();
  const lookbackHours = clampInteger(
    options.lookbackHours,
    DEFAULT_SYNC_LOOKBACK_HOURS,
    1,
    MAX_SYNC_LOOKBACK_HOURS
  );
  const from = options.from || new Date(now.getTime() - lookbackHours * 60 * 60 * 1000);
  const to = options.to || now;
  const rangeField = options.rangeField === 'creationDateRange' ? 'creationDateRange' : 'lastModifiedDateRange';

  return {
    page,
    pageSize: clampInteger(options.pageSize, DEFAULT_SYNC_PAGE_SIZE, 1, MAX_SYNC_PAGE_SIZE),
    [rangeField]: buildDateRangeFilter(from, to),
  };
}

async function syncRecentBokunBookings(options = {}, context = {}) {
  const pageSize = clampInteger(options.pageSize, DEFAULT_SYNC_PAGE_SIZE, 1, MAX_SYNC_PAGE_SIZE);
  const maxPages = clampInteger(options.maxPages, DEFAULT_SYNC_MAX_PAGES, 1, MAX_SYNC_MAX_PAGES);
  const processed = [];
  const failures = [];
  let totalFound = 0;

  for (let page = 0; page < maxPages; page += 1) {
    const query = buildBookingSearchQuery({ ...options, pageSize }, page);
    const response = await searchBookings(query);
    if (!response.ok) {
      const error = new Error(`Bókun booking search failed with status ${response.status}`);
      error.status = response.status;
      error.data = response.data;
      throw error;
    }

    const items = extractBookingSearchItems(response.data);
    ...
    for (const item of items) {
      const lookupId = bookingLookupId(item);
      try {
        const result = await processBokunBookingSearchItem(item, context);
        processed.push({
          ok: result.ok,
          delivery: result.delivery,
          lookupId: result.lookupId || lookupId,
