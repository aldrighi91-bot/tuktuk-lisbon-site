...
function parseDesiredDate(message = '') {
  ...
  const named = text.match(/\b(today|tomorrow|monday|tuesday|wednesday|thursday|friday|saturday|sunday|january|february|march|april|may|june|july|august|september|october|november|december)\b[^.,;!?]*/i);
...
function mergeLead(existing = {}, message = '', expectedField = '') {
  ...
  const clean = String(message).trim().replace(email || '', '').trim();
...
function buildTourRecommendation(tour, lead = {}, lang = 'en') {
  ...
  if (lang === 'pt') {
    return [
      ...
      note ? `\n${note}` : '',
      'Posso pedir data, horario e numero de pessoas para o Natanael verificar disponibilidade.'
    ].filter(Boolean).join('\n');
  }
  ...
  return [
    ...
    note || '',
    'I can help collect your date, preferred time and group size so Natanael can check availability.'
  ].filter(Boolean).join('\n');
}
...
function nextLeadQuestion(lead = {}, lang = 'en') {
  if (!lead.tourId) return lang === 'pt' ? 'Qual tour lhe interessa mais?' : 'Which tour are you most interested in?';
...
function availabilityReply(lead = {}, lang = 'en') {
  ...
  const intro = lang === 'pt'
    ? 'Posso recolher os detalhes e pedir ao Natanael para verificar disponibilidade. Nao vou prometer disponibilidade sem confirmacao.'
    : 'Let me check availability for you. I will collect the details for Natanael, but I will not promise availability until he confirms it.';
  ...
  return {
    ...
    quickReplies: question ? [] : ['Send request to Natanael', 'Add a question'],
...
function continueLeadCapture(lead = {}, lang = 'en', qualification = 'WARM') {
  ...
  if (question) {
    return {
      ...
      leadPatch: lead,
      qualification,
      nextExpectedField: nextExpectedLeadField(lead),
...
function handleConciergeMessage(input = {}) {
  ...
  if (expectedField && ['tourId', 'desiredDate', 'preferredTime', 'guests', 'name', 'email'].includes(expectedField)) {
