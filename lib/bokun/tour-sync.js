...
const DEFAULT_ALFAMA_EXPERIENCE_ID = 1272182;
...
function buildInstantAvailabilityRules(tour, existingAvailabilityRules = []) {
  const existing = Array.isArray(existingAvailabilityRules) ? existingAvailabilityRules[0] : null;
  return [{
    ...(existing?.id ? { id: existing.id } : {}),
    recurrenceRule: {
      ...(existing?.recurrenceRule?.id ? { id: existing.recurrenceRule.id } : {}),
...
function buildInstantCheckoutPayload(tour, currentComponents = {}) {
  ...
  return {
    ...
    startTimes: buildInstantStartTimes(tour, currentComponents.startTimes),
    availabilityRules: buildInstantAvailabilityRules(tour, currentComponents.availabilityRules),
    rates,
    pricing: buildPrivateGroupPricing(tour, defaults, rate),
  };
}
